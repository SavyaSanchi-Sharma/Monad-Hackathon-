...
contract ProfileMatcher is Ownable {
    ...
    function recordMatch(address user1, address user2, uint256 score) external onlyOwner {
        (,,, bool user1Registered) = userRegistry.users(user1);
        (,,, bool user2Registered) = userRegistry.users(user2);
        
        require(user1Registered, "User1 not registered");
        require(user2Registered, "User2 not registered");
